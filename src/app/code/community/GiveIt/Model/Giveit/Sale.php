 ...
 *
 */
class GiveIt_Model_Giveit_Sale extends Mage_Core_Model_Abstract {
    ...
    public function setObject(array $responseData) {
        ...
        if (array_key_exists(self::SALE_ITEMS_KEY, $responseData)) {
            foreach ($responseData[self::SALE_ITEMS_KEY] as $apiItem) {
                $item = Mage::getModel('giveit/giveit_sale_item');
                $this->_items[] = $item->setData($apiItem);
            }
        }

        $this->_buyer = $this->_mapDataToObject(self::SALE_BUYER_KEY, $responseData, 'giveit/giveit_sale_buyer');

        $this->_recipient = $this->_mapDataToObject(self::SALE_RECIPIENT_KEY, $responseData,
            'giveit/giveit_sale_recipient');

        $this->_address = $this->_mapDataToObject(self::SALE_ADDRESS_KEY, $responseData,
            'giveit/giveit_sale_address');
     ...
     *
     * @return GiveIt_Model_Giveit_Sale_Buyer
     */
     ...
     *
     * @return GiveIt_Model_Giveit_Sale_Shipping_Address
     */
